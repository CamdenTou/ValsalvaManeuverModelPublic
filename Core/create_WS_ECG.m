...
if isfield(raw_data,'Hraw')
    idx10 = find(Traw > 10, 1);
    if ~isempty(idx10) && idx10 > 1
        average_refractory_first10 = 60/mean(raw_data.Hraw(1:idx10));
    else
...
% --- Identify imputed R peaks
is_imputed = ~ismember(TR, TR_initial);

% Initialize interpolated S peak times and values
TS_interp = TS;
S_interp = S;

% Loop through imputed R peaks and interpolate S values
for i = find(is_imputed)'
    t_r = TR(i);  % current imputed R time

    % Find neighbors in original (non-imputed) data
    left_idx = find(~is_imputed & TR < t_r, 1, 'last');
    right_idx = find(~is_imputed & TR > t_r, 1, 'first');

    if ~isempty(left_idx) && ~isempty(right_idx)
        t1 = TR(left_idx); t2 = TR(right_idx);
        s1 = S(left_idx);  s2 = S(right_idx);
        ts1 = TS(left_idx); ts2 = TS(right_idx);

        % Linear interpolation in time and amplitude
        alpha = (t_r - t1) / (t2 - t1);
        s_interp = s1 + alpha * (s2 - s1);
        ts_interp = ts1 + alpha * (ts2 - ts1);

        S_interp(i) = s_interp;
        TS_interp(i) = ts_interp;
    else
        % Could not interpolate — mark as NaN or fallback
        S_interp(i) = NaN;
        TS_interp(i) = NaN;
    end
end

% Optional: remove any NaN-imputed S peaks (no interpolation possible)
valid_S = ~isnan(S_interp) & ~isnan(TS_interp);
TR = TR(valid_S);
R = R(valid_S);
S = S_interp(valid_S);
TS = TS_interp(valid_S);
...
% 
% % --- Identify imputed R peaks
% is_imputed = ~ismember(TR, TR_initial);  % Logical index
% 
% % --- Compute R-S amplitudes
% RS_amp = R - S;
% 
% % --- Get amplitudes for original (non-imputed) R-S pairs
% RS_orig = RS_amp(~is_imputed);
% RS_imputed = RS_amp(is_imputed);
% TR_imputed = TR(is_imputed);
% 
% % --- Compute robust central tendency of original amplitudes
% RS_med = median(RS_orig, 'omitnan');
% RS_iqr = iqr(RS_orig);
% 
% % --- Define bounds (e.g., within 1.5 IQR of the median)
% LB = RS_med - 1.25 * RS_iqr;
% UB = RS_med + 1.25 * RS_iqr;
% 
% % --- Filter imputed peaks based on amplitude deviation
% valid_imputed = (RS_imputed >= LB) & (RS_imputed <= UB);
% 
% % --- Combine valid original + filtered imputed peaks
% valid = true(size(TR));  % start with all true
% valid(is_imputed) = valid_imputed;  % only keep good imputations
% 
% % --- Apply mask to TR, R, S, TS
% TR = TR(valid);
% R  = R(valid);
% S  = S(valid);
% TS = TS(valid);
